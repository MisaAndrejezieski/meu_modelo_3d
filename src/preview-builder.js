import * as THREE from 'three';

export function buildSVGPreview(file, widthMm, cutDepth) {
  return new Promise((resolve) => {
    const group = new THREE.Group();
    const geometry = new THREE.BoxGeometry(widthMm, widthMm * 0.75, cutDepth);
    const material = new THREE.MeshStandardMaterial({ color: 0x4a5568, roughness: 0.5 });
    const mesh = new THREE.Mesh(geometry, material);
    mesh.position.z = -cutDepth / 2;
    group.add(mesh);

    resolve({
      group,
      metadata: { depthMm: cutDepth }
    });
  });
}

export function buildRasterPreview(file, widthMm, maxDepthMm, resolution) {
  return new Promise((resolve, reject) => {
    const reader = new FileReader();
    reader.onload = (event) => {
      const img = new Image();
      img.onload = () => {
        try {
          const canvas = document.createElement('canvas');
          const ctx = canvas.getContext('2d');

          const cols = resolution;
          const rows = Math.round(resolution * (img.height / img.width));
          canvas.width = cols;
          canvas.height = rows;

          ctx.drawImage(img, 0, 0, cols, rows);
          const imgData = ctx.getImageData(0, 0, cols, rows);
          const data = imgData.data;

          // 1. Binarização Inteligente: Isola o desenho/texto do fundo branco puro
          const grid = new Float32Array(cols * rows);
          const threshold = 0.92;

          for (let y = 0; y < rows; y++) {
            for (let x = 0; x < cols; x++) {
              const idx = y * cols + x;
              const pIdx = idx * 4;
              const r = data[pIdx];
              const g = data[pIdx + 1];
              const b = data[pIdx + 2];
              const lum = (0.299 * r + 0.587 * g + 0.114 * b) / 255;
              
              // 1.0 se for o corpo/traço/texto, 0.0 se for o fundo
              grid[idx] = lum < threshold ? 1.0 : 0.0;
            }
          }

          // 2. Transformada de Distância (Distance Transform - Algoritmo de Varredura Euclidiana)
          // Isso calcula exatamente a distância de cada pixel interno até a borda mais próxima.
          const distMap = new Float32Array(cols * rows);
          const INF = 1e6;

          // Inicialização
          for (let i = 0; i < cols * rows; i++) {
            distMap[i] = grid[i] === 0 ? 0 : INF;
          }

          // Varredura para frente
          for (let y = 0; y < rows; y++) {
            for (let x = 0; x < cols; x++) {
              const idx = y * cols + x;
              if (grid[idx] > 0) {
                let d = distMap[idx];
                if (x > 0) d = Math.min(d, distMap[y * cols + (x - 1)] + 1);
                if (y > 0) d = Math.min(d, distMap[(y - 1) * cols + x] + 1);
                if (x > 0 && y > 0) d = Math.min(d, distMap[(y - 1) * cols + (x - 1)] + 1.414);
                if (x < cols - 1 && y > 0) d = Math.min(d, distMap[(y - 1) * cols + (x + 1)] + 1.414);
                distMap[idx] = d;
              }
            }
          }

          // Varredura para trás
          for (let y = rows - 1; y >= 0; y--) {
            for (let x = cols - 1; x >= 0; x--) {
              const idx = y * cols + x;
              if (grid[idx] > 0) {
                let d = distMap[idx];
                if (x < cols - 1) d = Math.min(d, distMap[y * cols + (x + 1)] + 1);
                if (y < rows - 1) d = Math.min(d, distMap[(y + 1) * cols + x] + 1);
                if (x < cols - 1 && y < rows - 1) d = Math.min(d, distMap[(y + 1) * cols + (x + 1)] + 1.414);
                if (x > 0 && y < rows - 1) d = Math.min(d, distMap[(y + 1) * cols + (x - 1)] + 1.414);
                distMap[idx] = d;
              }
            }
          }

          // Encontra a distância máxima para normalização
          let maxDist = 1.0;
          for (let i = 0; i < distMap.length; i++) {
            if (distMap[i] !== INF && distMap[i] > maxDist) {
              maxDist = distMap[i];
            }
          }

          // 3. Construção da Malha 3D com Perfil Orgânico e de Letras Cursivas
          const heightMm = widthMm * (rows / cols);
          const geometry = new THREE.PlaneGeometry(widthMm, heightMm, cols - 1, rows - 1);
          const positions = geometry.attributes.position;

          for (let i = 0; i < positions.count; i++) {
            const d = distMap[i];
            let z = 0;

            if (d !== INF && d > 0) {
              // Normaliza entre 0 e 1 a partir da borda
              const t = Math.min(1.0, d / maxDist);
              
              // Perfil matemático em Cúpula (Dome) suave para curvas e relevos femininos/orgânicos
              // A função seno garante transição suave nas bordas (Z=0) e volume arredondado no centro
              const profile = Math.sin(t * (Math.PI / 2));
              z = profile * maxDepthMm;
            } else {
              z = 0; // Fundo estritamente plano
            }

            positions.setZ(i, z);
          }

          geometry.computeVertexNormals();

          const material = new THREE.MeshStandardMaterial({
            color: 0xc27803,
            roughness: 0.3,
            metalness: 0.05,
            side: THREE.DoubleSide,
            flatShading: false
          });

          const mesh = new THREE.Mesh(geometry, material);
          const group = new THREE.Group();
          group.add(mesh);

          resolve({
            group,
            metadata: {
              imageData: data,
              cols,
              rows,
              heightMm,
              maxHeightMm: maxDepthMm
            }
          });
        } catch (err) {
          reject(err);
        }
      };
      img.onerror = reject;
      img.src = event.target.result;
    };
    reader.onerror = reject;
    reader.readAsDataURL(file);
  });
}