    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        try {
          ...
          // 1. Extrai matriz de luminância limpa
          ...
          for (let y = 0; y < rows; y++) {
            for (let x = 0; x < cols; x++) {
              ...
              const g = data[pIdx + 1];
              const b = data[pIdx + 2];
              // Luminância padrão invertida (0 = branco/fundo, 1 = escuro/desenho)
              const lum = (0.299 * r + 0.587 * g + 0.114 * b) / 255;
              gray[idx] = 1.0 - lum; 
            }
          }

          // 2. Filtro Sobel leve para realçar contornos, curvas e letras cursivas sem borrar
          ...
          for (let i = 0; i < positions.count; i++) {
            ...
            // Se o pixel pertencer ao desenho/texto (com margem para ignorar o fundo branco)
            if (val > 0.05) {
              // Curva de relevo proporcional limpa: mantém os detalhes e dá volume encorpado
              const profile = Math.pow(val, 1.1);
              z = profile * maxDepthMm;
            } else {
              z = 0; // Fundo estritamente plano na cota zero
            }
